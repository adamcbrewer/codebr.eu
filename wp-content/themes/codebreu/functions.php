...
function custom_pagination($pages = '', $range = 2) {
	...
	if(1 != $pages) {
		...
		if($paged > 1 && $showitems < $pages) echo "<a class=\"page-prev\" rel=\"prev\" href='".get_pagenum_link($paged - 1)."'>Previous Page</a>";

		echo '<ul>';
			for ($i=1; $i <= $pages; $i++) {
				if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems )) {
					echo '<li class="page-item">';	
					echo ($paged == $i) ? "<span class='page-current'>".$i."</span>" : "<a href='".get_pagenum_link($i)."' class='page-link' >".$i."</a>";
					echo '</li>';
				}
			}
		echo '</ul>';

		if ($paged < $pages && $showitems < $pages) echo "<a class=\"page-next\" rel=\"next\" href='".get_pagenum_link($paged + 1)."'>Next Page</a>";
...
function excerpt_read_more_link($output) {
	global $post;
	$text_to_replace = ' <a href="'. get_permalink() . '">' . __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'io' ) . '</a>';
	$new_output = str_replace($text_to_replace, '', $output);

	return $new_output . '<p class="post-more"><a href="'. get_permalink($post->ID) . '">read on &#10095;</a></p>';
}
